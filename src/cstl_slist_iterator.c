...
slist_iterator_t _create_slist_iterator(void)
{
    ...
    _GET_SLIST_ITERATOR_TYPE(it_iter) = _FORWARD_ITERATOR;
    _GET_SLIST_CONTAINER_TYPE(it_iter) = _SLIST_CONTAINER;
    _GET_CONTAINER(it_iter) = NULL;
    _GET_SLIST_COREPOS(it_iter) = NULL;
...
bool_t _slist_iterator_equal(slist_iterator_t it_first, slist_iterator_t it_second)
{
    ...
    assert(_slist_iterator_belong_to_slist(_GET_SLIST_CONTAINER(it_first), it_first));
    assert(_slist_iterator_belong_to_slist(_GET_SLIST_CONTAINER(it_second), it_second));

    return _GET_SLIST_COREPOS(it_first) == _GET_SLIST_COREPOS(it_second) ? true : false;
}
...
bool_t _slist_iterator_before(slist_iterator_t it_first, slist_iterator_t it_second)
{
    ...
    if(_GET_SLIST_COREPOS(it_first) == _GET_SLIST_COREPOS(it_second))
    {
        return false;
    }
    ...
    return _GET_SLIST_COREPOS(it_second) == NULL ? true : false;
}
