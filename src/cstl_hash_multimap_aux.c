/*
 *  The implementation of hash_multimap auxiliary functions.
 ...
 * Test the two pair is have the same type.
 */
bool_t _hash_multimap_same_pair_type_ex(const pair_t* cppair_first, const pair_t* cppair_second)
{
    assert(cppair_first != NULL);
    assert(cppair_second != NULL);
    assert(_pair_is_inited(cppair_first) || _pair_is_created(cppair_first));
...
void _hash_multimap_value_compare(const void* cpv_first, const void* cpv_second, void* pv_output)
{
    ...
    *(bool_t*)pv_output = ppair_first->_t_typeinfofirst._pt_type->_t_typesize;
    if (ppair_first->_bfun_mapkeycompare != NULL) /* external key less */
    {
