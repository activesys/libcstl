 ...
 * test _quick_sort
 */
UT_CASE_DEFINATION(_quick_sort)

static void _quick_sort_int_greater(const void* cpv_first, const void* cpv_second, void* pv_output)
{
...
void test__quick_sort__empty_less(void** state)
{
    list_t* plist = create_list(int);

    list_init(plist);
    assert_true(list_size(plist) == 0);
    expect_assert_failure(_quick_sort(list_end(plist), list_end(plist), NULL));
    assert_true(list_size(plist) == 0);
...
void test__quick_sort__empty_greater(void** state)
{
    ...
    list_init(plist);
    assert_true(list_size(plist) == 0);
    expect_assert_failure(_quick_sort(list_end(plist), list_end(plist), _quick_sort_int_greater));
    assert_true(list_size(plist) == 0);
...
void test__quick_sort__2_equal_less(void** state)
{
    ...
    _quick_sort(list_end(plist), list_end(plist), NULL);
    assert_true(list_size(plist) == 2);
    it_iter = list_begin(plist);
    assert_true(*(int*)iterator_get_pointer(it_iter) == 0);
    it_iter = iterator_next(it_iter);
...
void test__quick_sort__2_equal_greater(void** state)
{
    ...
    list_init_n(plist, 2);
    assert_true(list_size(plist) == 2);
    _quick_sort(list_end(plist), list_end(plist), _quick_sort_int_greater);
    assert_true(list_size(plist) == 2);
    it_iter = list_begin(plist);
    assert_true(*(int*)iterator_get_pointer(it_iter) == 0);
    it_iter = iterator_next(it_iter);
